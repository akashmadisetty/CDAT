"""
Train RFM Customer Segmentation Model on Pair 1 Gold Source Data
Uses the same methodology as week2/improved_train_all_domains.py
EXACT SAME architecture as train_uk_source_model.py
"""
...
# Configuration
PAIR1_SOURCE_CONFIG = {
    'name': 'E-commerce Gold Membership - Source Domain',
    'rfm_file': '../../data/processed/ecommerce/pair1_gold_source_RFM.csv',
    'description': 'Gold membership customers (source domain for pair 1)',
    'experiment': 'Pair 1: Gold (source) -> Silver (target)'
}
...
def train_pair1_gold_source_model():
    """
    Train RFM clustering model for Pair 1 Gold source domain with improved analysis
    ...
    """
    print("\n" + "="*80)
    print(f"TRAINING: {PAIR1_SOURCE_CONFIG['name']}")
    print(f"   Description: {PAIR1_SOURCE_CONFIG['description']}")
    print(f"   Experiment: {PAIR1_SOURCE_CONFIG['experiment']}")
    print("="*80)
    
    # Load RFM data
    print(f"\nLoading: {PAIR1_SOURCE_CONFIG['rfm_file']}")
    df_rfm = pd.read_csv(PAIR1_SOURCE_CONFIG['rfm_file'])
    ...
    # Plot elbow curve
    plot_elbow_curve(results, save_path='plots/pair1_gold_source_elbow_curve.png')
    ...
    profiles.to_csv('results/pair1_gold_source_segment_profiles.csv')
    print(f"\nSaved: results/pair1_gold_source_segment_profiles.csv")
    ...
    df_segments.to_csv('results/pair1_gold_source_customer_segments.csv', index=False)
    print(f"Saved: results/pair1_gold_source_customer_segments.csv")
    ...
    model.plot_rfm_3d(df_clean, 
                      title=f"RFM Segments: {PAIR1_SOURCE_CONFIG['name']}", 
                      save_path='plots/pair1_gold_source_rfm_3d.png')
    
    # Segment profiles
    model.plot_segment_profiles(profiles, 
                                title=f"Segment Profiles: {PAIR1_SOURCE_CONFIG['name']}", 
                                save_path='plots/pair1_gold_source_segment_profiles.png')
    
    # Customer distribution
    model.plot_segment_distribution(profiles, 
                                   title=f"Customer Distribution: {PAIR1_SOURCE_CONFIG['name']}", 
                                   save_path='plots/pair1_gold_source_distribution.png')
    
    # Save model
    model.save_model('models/pair1_gold_source_rfm_kmeans_model.pkl')
    ...
    summary = {
        'experiment': PAIR1_SOURCE_CONFIG['experiment'],
        'domain': PAIR1_SOURCE_CONFIG['name'],
    ...
    summary_df.to_csv('results/pair1_gold_source_training_summary.csv', index=False)
    print(f"\nSaved: results/pair1_gold_source_training_summary.csv")
    
    print("\n" + "="*80)
    print("DELIVERABLES CREATED:")
    print("="*80)
    print("  models/pair1_gold_source_rfm_kmeans_model.pkl - Trained model")
    print("  results/pair1_gold_source_segment_profiles.csv - Segment statistics")
    print("  results/pair1_gold_source_customer_segments.csv - Customer assignments")
    print("  results/pair1_gold_source_training_summary.csv - Performance metrics")
    print("  plots/pair1_gold_source_elbow_curve.png - K selection analysis")
    print("  plots/pair1_gold_source_rfm_3d.png - 3D RFM visualization")
    print("  plots/pair1_gold_source_segment_profiles.png - Segment profiles")
    print("  plots/pair1_gold_source_distribution.png - Customer distribution")
    
    print("\nNext Steps:")
    print("  1. Review segment profiles and customer assignments")
    print("  2. Validate segmentation quality with business experts")
    print("  3. Test transfer learning to Silver target domain")
    print("  4. Compare with Silver target segmentation")
...
def main():
    """
    Main training pipeline for Pair 1 Gold source domain
    """
    print("\n" + "="*40)
    print("RFM CUSTOMER SEGMENTATION")
    print("PAIR 1 GOLD SOURCE DOMAIN TRAINING")
    print("="*40)
    ...
    # Train model
    try:
        summary = train_pair1_gold_source_model()
        ...
        print(f"\nERROR: File not found - {PAIR1_SOURCE_CONFIG['rfm_file']}")
