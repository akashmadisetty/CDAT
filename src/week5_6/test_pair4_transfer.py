"""
Test Transfer Learning: Pair 4 Source -> Target
Tests both direct transfer and fine-tuned transfer based on transferability analysis
EXACT SAME architecture as test_transfer_learning.py
CLI Recommendation: Fine-tune with 57% of target data (LOW transferability, score 0.67)
"""
...
# Configuration
FINE_TUNE_PERCENTAGE = 0.57  # 57% as recommended by transferability analysis
...
def load_trained_model(model_path):
    """Load the trained Pair 4 source model"""
    print(f"\nLoading trained Pair 4 source model from: {model_path}")
...
def split_target_data(df, fine_tune_pct=0.57):
    """
    ...
    fine_tune_pct : float
        Percentage of data for fine-tuning (0.57 = 57%)
...
def test_direct_transfer(source_model_data, df_test):
    ...
    print("="*80)
    print("\nApplying Pair 4 source model directly to target test data...")
...
def test_finetuned_transfer(source_model_data, df_finetune, df_test):
    ...
    """
    print("\n" + "="*80)
    print("TEST 2: FINE-TUNED TRANSFER (57% Fine-tuning)")
    print("="*80)
    print("\nFine-tuning Pair 4 source model with target data...")
    ...
    print(f"\nFine-tuning with {len(df_finetune)} target customers...")
...
def compare_results(direct_metrics, finetuned_metrics):
    ...
    if better_count >= 2:
        ...
        print(f"  {better_count}/3 metrics improved with 57% fine-tuning data")
        print("\nRecommendation: Use fine-tuned model for target domain")
    elif better_count == 0:
...
def save_results(direct_metrics, finetuned_metrics, df_test, direct_labels, finetuned_labels):
    ...
    if direct_metrics and finetuned_metrics:
        ...
        comparison.to_csv('results/pair4_transfer_comparison.csv', index=False)
        print(f"\nSaved: results/pair4_transfer_comparison.csv")
    
    # Save predictions
    df_predictions = df_test.copy()
    df_predictions['direct_transfer_cluster'] = direct_labels
    df_predictions['finetuned_transfer_cluster'] = finetuned_labels
    df_predictions.to_csv('results/pair4_test_predictions.csv', index=False)
    print(f"Saved: results/pair4_test_predictions.csv")
...
def main():
    ...
    """
    print("\n" + "="*80)
    print("TRANSFER LEARNING TEST: Pair 4 Source -> Target")
    print("="*80)
    print(f"\nConfiguration:")
    print(f"  Fine-tuning percentage: {FINE_TUNE_PERCENTAGE*100:.0f}% (as recommended by CLI)")
    print(f"  CLI Transferability Score: 0.6709 (LOW)")
    ...
    # Load trained Pair 4 source model
    model_path = '../week2/models/domain_pair4_rfm_kmeans_model.pkl'
    source_model_data = load_trained_model(model_path)
    
    # Load Pair 4 target data
    print(f"\nLoading Pair 4 target RFM data...")
    df_target = pd.read_csv('../week2/domain_pair4_target_RFM.csv')
    
    print(f"Loaded {len(df_target)} target customers")
    
    # Split target data
    df_finetune, df_test = split_target_data(df_target, FINE_TUNE_PERCENTAGE)
    ...
    print("="*80)
    print("\nDeliverables:")
    print("  results/pair4_transfer_comparison.csv - Performance comparison")
    print("  results/pair4_test_predictions.csv - Test set predictions")
