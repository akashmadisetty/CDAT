"""
Test Transfer Learning: Pair 1 Gold Source -> Silver Target
...
def test_direct_transfer(source_model_data, df_test):
    ...
    print("="*80)
    print("\nApplying Gold source model directly to Silver test data...")
...
def test_finetuned_transfer(source_model_data, df_finetune, df_test):
    ...
    print("="*80)
    print("\nFine-tuning Gold source model with Silver data...")
    ...
    print(f"\nFine-tuning with {len(df_finetune)} Silver customers...")
...
def compare_results(direct_metrics, finetuned_metrics):
    ...
    if better_count >= 2:
        ...
        print("\nRecommendation: Use fine-tuned model for Silver target domain")
    elif better_count == 0:
...
def save_results(direct_metrics, finetuned_metrics, df_test, direct_labels, finetuned_labels):
    ...
    if direct_metrics and finetuned_metrics:
        ...
        comparison.to_csv('results/pair1_transfer_comparison.csv', index=False)
        print(f"\nSaved: results/pair1_transfer_comparison.csv")
    
    # Save predictions
    df_predictions = df_test.copy()
    df_predictions['direct_transfer_cluster'] = direct_labels
    df_predictions['finetuned_transfer_cluster'] = finetuned_labels
    df_predictions.to_csv('results/pair1_test_predictions.csv', index=False)
    print(f"Saved: results/pair1_test_predictions.csv")
...
def main():
    ...
    """
    print("\n" + "="*80)
    print("TRANSFER LEARNING TEST: Pair 1 Gold (Source) -> Silver (Target)")
    print("="*80)
    print(f"\nConfiguration:")
    ...
    # Load Silver target data
    print(f"\nLoading Silver target RFM data...")
    df_silver = pd.read_csv('../../data/processed/ecommerce/pair1_silver_target_RFM.csv')
    
    print(f"Loaded {len(df_silver)} Silver customers")
    
    # Split target data
    df_finetune, df_test = split_target_data(df_silver, FINE_TUNE_PERCENTAGE)
    ...
    print("="*80)
    print("\nDeliverables:")
    print("  results/pair1_transfer_comparison.csv - Performance comparison")
    print("  results/pair1_test_predictions.csv - Test set predictions")
